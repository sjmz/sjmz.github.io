...
In practice it means that there are three additional bytes somewhere.
...
This is the definition of `struct rtnl_link_ifmap` with the corresponding sizes:
...
For this reason, 4 bytes are added as padding to `struct rtnl_link_ifmap` so its size becomes a multiple of 8 bytes.
